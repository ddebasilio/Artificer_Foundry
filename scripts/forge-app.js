...
export class ForgeApp extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static DEFAULT_OPTIONS = {
        window: { title: "Item Forge", icon: "fas fa-hammer", resizable: true },
        classes: ["artificer-foundry", "forge-app"],
        position: { width: 980, height: 700 },
    };

    static PARTS = {
        forge: { template: "modules/artificer-foundry/templates/forge-app.hbs" }
    };
    ...
    async _prepareContext(options) {
        const allRecipes = window.ArtificerFoundry.forgeRecipeManager.getRecipesForActor(this.actor);
        ...
        const recipes = allRecipes.filter(r => {
            ...
            return { ...r, craftingTimeLabel: formatForgeCraftingTime(ct.days), craftingCost: ct.cost };
        });
        ...
        if (this.selectedRecipeId) {
            ...
            if (selectedRecipe) {
                const ct = getForgeCraftingTime(selectedRecipe.rarity, isSmith);
                selectedRecipe = { ...selectedRecipe, craftingTimeLabel: formatForgeCraftingTime(ct.days), craftingCost: ct.cost };
        ...
        if (this.actor && selectedRecipe) {
            ...
            for (const ing of selectedRecipe.ingredients) {
                ...
                for (const s of getForgeSubstitutes(ing.name, selectedRecipe.rarity)) allIngNames.add(s.toLowerCase());
            }
        ...
        return {
            ...
            filterLearned: this.filterLearned, filterCategory: this.filterCategory,
            searchQuery: this.searchQuery, isSmith, crafting: this._crafting,
        };
    }
    ...
    _onRender(context, options) {
        ...
        el.querySelectorAll('.filter-category').forEach(sel => sel.addEventListener('change', this._onFilterChange.bind(this)));
    ...
    _onFilterChange(event) {
        ...
        if (el.classList.contains('filter-category')) this.filterCategory = el.value;
        this.render();
    }
    ...
    async _onDropIngredient(event) {
        ...
        if (!this.selectedRecipeId) { ui.notifications.warn("Select a blueprint first."); return; }
        try {
        ...
        } catch (err) { console.error("Artificer Foundry | Forge drop error:", err); }
    }
    ...
    _addIngredientFromItem(item) {
        ...
        const totalInInventory = (this.actor?.items?.contents ?? [])
            .filter(i => canForgeSubstitute(i.name, required.name, recipe.rarity))
        ...
        ui.notifications.info(`Added ${item.name} to the forge.`);
        this.render();
    }
    ...
    async _onCraftItem(event) {
        ...
        for (const ing of recipe.ingredients) {
            ...
            const matching = this.actor.items.filter(i => canForgeSubstitute(i.name, ing.name, recipe.rarity));
